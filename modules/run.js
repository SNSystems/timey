...
function spawn_process (bin_dir, executable, args, verbose) {
    ...
    const log = (channel) => {
        const name = basename (exe_path, '.exe');
...
exports.runner = (bin_dir, work_dir, repo_name, verbose = false) => {
    return {
         ...
         */
        rld_gen: (num_modules, num_external_symbols, num_linkonce_symbols) => spawn_process (bin_dir, 'rld-gen', [
         ...
         * @returns {Promise<*>} A promise which is resolved once the process exits.
         */
        repo2obj: (ticket_file, output_file) => spawn_process (bin_dir, 'repo2obj', [
            '-o', output_file,
            '--repo', join (work_dir, repo_name),
            ticket_file
        ], verbose),
         ...
         * @returns {Promise<*>} A promise which is resolved once the process exits.
         */
        rld: (ticket_files, output_file) => spawn_process (bin_dir, 'rld', [
            '-o', output_file,
            '--repo', join (work_dir, repo_name)
        ].concat (ticket_files), verbose),
         ...
         * @returns {Promise<*>} A promise which is resolved once the process exits.
         */
        lld: (object_files, output_file) => spawn_process (bin_dir, 'ld.lld', [
            '-o', output_file
        ].concat (object_files), verbose),
    };
};
